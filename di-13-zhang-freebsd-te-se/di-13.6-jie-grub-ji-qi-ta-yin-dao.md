...
# gpart add -b 40 -s 260M -t ms-basic-data -i 1 nda0
nda0p1 added
# gpart show
=>       40  125829040  nda0  GPT  (60G)
...
格式化为 fat32（`/dev/nda0p1` 必须保持卸载状态）。但是若指定扇区似乎会影响 4K 对齐？

```sh
# newfs_msdos -F /dev/nda0p1
...
设装有 Windows 的硬盘为 `ada0`，FreeBSD 的硬盘为 `nda0`。本文是删掉了 nda0，即 BSD 系统安装生成的 EFI 分区（不知道为什么 FreeBSD 的 EFI 文件系统是 Fat16）。将 FreeBSD 的引导文件放到了 `ada0` 硬盘下。
...
目前测试 `grub` 无法直接引导 FreeBSD 的内核从而启动系统，只能通过 GRUB 的 chainload 机制（例如在配置中使用 `chainloader +1`）间接引导。
