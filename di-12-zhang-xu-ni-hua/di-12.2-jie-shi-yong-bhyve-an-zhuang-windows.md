...
```sh
loader="uefi" # 不支持 UEFI 的 Windows 无法启动，例如 Windows XP；但 Windows 7 支持 UEFI
graphics="yes" # 设置虚拟机在启动时暂停，直至 VNC 客户端连接
xhci_mouse="yes"
cpu=2 # 分配 CPU 数量，建议适当增加
memory=4G # 分配内存大小
...
utctime="no" # 指定 Windows 不使用 UTC 时间，避免时差 8 小时
