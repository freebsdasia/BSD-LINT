...
VirtualBox 在 FreeBSD 上有多个版本，截止本文撰写时，有：`emulators/virtualbox-ose-legacy`（5.X）、`emulators/virtualbox-ose`（6.X）、`emulators/virtualbox-ose-70` 和 `emulators/virtualbox-ose-71`，其中都有无 X11 的 `-nox11` 版本（例如 `emulators/virtualbox-ose-71-nox11`，无图形界面，纯命令行，命令是 `VBoxManage`）。
